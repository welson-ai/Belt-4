...
impl LiquidationEngine {
    pub fn initialize(env: Env, lending_pool_address: Address) {
        ...
        env.storage()
            .instance()
            .set(&LENDING_POOL, &lending_pool_address);
    }
...
mod tests {
    ...
    fn test_liquidation_triggered() {
        ...
        let is_liquidatable =
            LiquidationEngine::has_liquidatable_position(env.clone(), borrower.clone());
    ...
    fn test_constants() {
        ...
        assert_eq!(
            LiquidationEngine::get_liquidation_threshold(env.clone()),
            120
        );
